...
echo Hey! you can type windows commands too! Just type "wcommandsmode"!
echo Code 0
echo.                                                                              
echo.                                                              
echo.                                    %date%
...
%speech% "Greetings %username%! It is currently %timeminute% minutes past %timehour%."
::echo The current time is %time% sir | clip
::start scripts\nircmd.exe speak text ~$clipboard$  
...
echo.
if "%C%"=="startwsl" C:\Users\email\Desktop\MAT\tools\wsl.bat goto start
if "%C%"=="startbruteforcer" echo plugin directory C:\Users\email\Desktop\MAT\tools\BruteForcer_091\Client\Plugins start C:\Users\email\Desktop\MAT\tools\BruteForcer_091\Client\BFC.exe goto start
if "%C%"=="startipscanner" C:/users/email/Desktop/MAT/tools/Networkipscanner.bat goto start
if "%C%"=="speech" echo Acess Denied, HACKSTOP DETECTED
if "%C%"=="wcommandsmode" goto wcmdstrt
if "%C%"=="run admin" LOLOUDADMIN.bat
if "%C%"=="hello" goto loloud
...
if "%C%"=="restart" cls && call LOLOUD.bat
...
if "%C%"=="OpenRoblox" goto open_roblox
if "%C%"=="Chrometime" goto chrometime
...
echo INFO PLS READ.
echo.
echo This mode is basicly a old version of LOLOUD
loloudcmd.bat

:cctv
C:\Users\email\Desktop\MAT\camera\cam.html
goto start

:matrix
C:\Users\email\Desktop\MAT\Matrix-effect\Mat.html
...
:restarttwo
C:\Users\email\Desktop\MAT\ALERT.bat

:open_roblox
C:\Users\email\AppData\Local\Roblox\Versions\version-0137f7d4848f436a\RobloxPlayerBeta.exe"
%speech% Opening...
echo Opening...
...
:chrometime
C:\Users\email\Desktop\Google_Chrme.lnk
...
echo startbruteforcer HACKING TOOL
echo startipscanner HACKING TOOL
echo loloud (also "hello there loloud")
echo introduce
echo mute
echo unmute
echo killtask
...
if "%killpri%"=="Robert Downey Jr. 1 1 0" echo Code accepted. && echo. && (set /p killsec=Does the second commanding officer agree? ) && goto killsec
if "%killpri%"=="Raqeeb Ameen 1 1 0" echo Code accepted. && echo. && (set /p killsec=Does the second commanding officer agree? ) && goto killsec
...
echo. ERROR :(
echo.
echo. LOLOUD has crashed please restart the system
echo.
echo.
echo.
echo.
...
set /p alarmtime=(Current time is %time%) : 
...
set /p alarmoption=Extra message (if not, leave empty):
echo.
echo Options:
echo 'No resume' (no)
...
:lock
echo Locking my doors now, %username%.
%speech% "Locking my doors now, %username%."
set /a delay = %1 + 3
